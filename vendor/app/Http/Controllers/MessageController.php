...
//        print_r($message);
    ...
    public function message_updateoperator(Request $request){
        $user = Auth::User();
        $message = \App\Message::join('ticket', 'message.ID_TECH', '=', 'ticket.ID_TECH')->
                where('ticket.ID_TECH',$user->id)->where('message.Status',1)->get();
        echo '('.count($message).')';
    }
    ...
    public function message_updatecliente(Request $request){
        $user = Auth::User();
        $message = \App\Message::join('ticket', 'message.ID_TECH', '=', 'ticket.ID_TECH')->
                where('ticket.ID_TECH',$user->id)->where('ticket.Status',1)->get();
        echo '('.count($message).')';
    }
    ...
    public function message_operator()
    {
        $user = Auth::User();
        $message = \App\Message::join('ticket', 'message.ID_TECH', '=', 'ticket.ID_TECH')->
                where('ticket.ID_TECH',$user->id)
                ->select('message.id', 'message.Status', 'message.CONTENT','message.ID_TECH')->get();
